...
$(document).ready(function(){
	...
	$("#filtertype").change(function(){
        $('#filtervalue').show().prop("disabled", false).val('');
        $('#location_id').hide().prop("disabled", true);
		$filter_val = $(this).val();
		...
		} else if ($filter_val == 'location') {
            $('#filtervalue').hide().prop("disabled", true);
            $('#location_id').show().prop("disabled", false);
        }		
	});
...
<script>
    ...
    function sendSms(){
		var tmplId = $('#sel_sms').val();
		if(tmplId != ''){
			var ntContent = $('#sel_sms').find(':selected').attr('data-cnt'); 
			$('.notification_cnt').css('display','block');
			$('#smsContent').html('<div class="form_grid_12 notification_container" ><h3>SMS Template Preview</h3><p>'+ntContent+'</p></div>');
		} else {
			$('.notification_cnt').css('display','none');
			$('#smsContent').html('');
		}
...
<div id="content">
    <div class="grid_container">
								...
								<div class="widget_top filter_widget">
									...
									<form method="GET" id="filter_form" action="admin/notification/display_notification_user_list" accept-charset="UTF-8">	
										<select class="form-control" id="filtertype" name="type" tabindex="1">
                                            ...
                                            <option value="location" data-val="location" <?php if(isset($type)){if($type=='location'){ echo 'selected="selected"'; } }?>><?php if ($this->lang->line('admin_users_users_list_location') != '') echo stripslashes($this->lang->line('admin_users_users_list_location')); else echo 'Location'; ?></option>
										</select>
            ...
            <div class="widget_wrap">
                <div class="widget_top">
                    ...
                    <div style="float: right;line-height:40px;padding:0px 10px;height:39px;">
                        ...
                        <div class="btn_30_light" style="height: 29px;">
                            <a class="o-modal" data-content="send_sms" style="background:#6bba70 !important;border: 1px solid #6bba70" href="javascript:void(0)">
                                <span class="icon accept_co"></span>
                                <span style="border-left: 1px solid green;" class="btn_link"><?php if ($this->lang->line('admin_notification_send_sms') != '') echo stripslashes($this->lang->line('admin_notification_send_sms')); else echo 'Send SMS'; ?></span>
                            </a>
                        </div>
                    </div>
                </div>
                            ...
                            if ($usersList->num_rows() > 0) {
                                foreach ($usersList->result() as $row) {
                                    ...
                                    <tr>
										...
										<td class="center">
                                            <?php if(isset($row->device_type))echo get_language_value_for_keyword($row->device_type,$this->data['langCode']); ?>
                                        </td>
                                        <td class="center">
        ...
        <div id="send_sms" style="display:none;">
            <h3><?php if ($this->lang->line('admin_notification_send_sms') != '') echo stripslashes($this->lang->line('admin_notification_send_sms')); else echo 'Send SMS'; ?></h3>
            <?php
            $attributes = array('class' => 'form_container left_label', 'id' => 's_sms_form', 'enctype' => 'multipart/form-data', 'method' => 'post');
            echo form_open_multipart('admin/notification/send_sms_to_users', $attributes);
            ?>
            <ul>
                <li>
                    <input name="userIds" type="hidden" class="tdesc"  value=""/>
					<input name="user_type" type="hidden" value="user"/>
                    <div class="form_grid_12">
                        <label class="field_title" for="media_title" style="width: 20%;"><?php if ($this->lang->line('admin_notification_choose_sms_template') != '') echo stripslashes($this->lang->line('admin_notification_choose_sms_template')); else echo 'Choose SMS Template'; ?> <span class="req">*</span></label>
                        <div class="form_input select-template-container">
                          <select id="sel_sms" onchange="sendSms();" class="chzn-select1 required" name="sms_id" tabindex="1" style="width: 350px; display: none;" data-placeholder="Select Model Type" required>
							<option value=""><?php if ($this->lang->line('admin_notification_select_sms_template') != '') echo stripslashes($this->lang->line('admin_notification_select_sms_template')); else echo 'Select SMS Template....'; ?></option>
                            <?php					
								foreach ($template_details->result() as $temp_detail) {
									if ($temp_detail->notification_type == 'sms') {
							?>
							<option value="<?php echo $temp_detail->news_id; ?>" data-cnt="<?php echo htmlentities($temp_detail->message['sms_body']) ?>"><?php echo $temp_detail->message['title'] ?></option>		
                            <?php
                                    }
								}
							?>
						...
						<button type="submit" id="send-sms-button" class="btn_small btn_blue " tabindex="5"><span><?php if ($this->lang->line('admin_notification_send') != '') echo stripslashes($this->lang->line('admin_notification_send')); else echo 'Send'; ?></span></button>
                        <span id="sms-template-error" class="error" style="display: none;"></span>
                    </div>
                </li>
