            ...
            <div class="widget_wrap">
                ...
                ?>      
                        <ul>
                            ...
                            <li>
                                <div class="form_grid_12">
                                    <label class="field_title" for="Distanceunit">Distance Unit<span class="req">*</span></label>
                                    <div class="form_input">
                                        <select class="required" style="height: 31px; width: 51%;" id="distance_unit" name="distance_unit" tabindex="1" data-placeholder="Choose distance unit">
                                        <option value="">Choose distance unit...</option>
                                            <option value="km-Kilometer" <?php if($form_mode) if(isset($locationdetails->row()->distance_unit)) if($locationdetails->row()->distance_unit == 'km')echo 'selected="selected"';?>>Kilometer</option>
                                            <option value="mi-Miles" <?php
                                            if($form_mode) if(isset($locationdetails->row()->distance_unit)) if($locationdetails->row()->distance_unit == 'mi') echo 'selected="selected"';?>>Miles</option>
                                        </select>
                                    </div>
                                </div>
                           </li>
                            ...
                            <?php 
                                ...
                                if (isset($locationdetails)) {
                                    if (isset($locationdetails->row()->welcome_amount)) {
                                        $welcome_amount = $locationdetails->row()->welcome_amount;
                                    }
                                    if (isset($locationdetails->row()->currency)) {
                                        $currency_arr[] = $locationdetails->row()->currency;
                                    }
                                    if (isset($locationdetails->row()->currency_symbol)) {
                                        $currency_arr[] = $locationdetails->row()->currency_symbol;
                                    }
                            ...
                            <li>
                                <div class="form_grid_12">
                                    <label class="field_title" for="welcome_amount">WELCOME AMOUNT <span class="req">*</span></label>
                                    <div class="form_input">
                                        <input name="welcome_amount" id="welcome_amount" type="text" tabindex="6" class="large required number tipTop positiveNumber" title="WELCOME AMOUNT" value="<?php echo $welcome_amount; ?>"/> <?php echo $currency_str; ?>
                                    </div>
                                </div>
                            </li>
                            <?php 
                                $referral_credit = 0;
                                if (isset($locationdetails) && isset($locationdetails->row()->referral_credit)) {
                                    $referral_credit = $locationdetails->row()->referral_credit;
                                }
                            ?>
                            <li>
                                <div class="form_grid_12">
                                    <label class="field_title" for="referral_credit">AMOUNT PER REFERRAL <span class="req">*</span></label>
                                    <div class="form_input">
                                        <input name="referral_credit" id="referral_credit" type="text" tabindex="6" class="large required number tipTop positiveNumber" title="WELCOME AMOUNT" value="<?php echo $referral_credit; ?>"/> <?php echo $currency_str; ?>
                                    </div>
                                </div>
                            </li>

                            <?php
                                $instant_selected = $on_first_ride_selected = '';
                                $referral_credit_type = null;
                                if (isset($locationdetails) && isset($locationdetails->row()->referral_credit_type)) {
                                    $referral_credit_type = $locationdetails->row()->referral_credit_type;
                                }
                                if ($referral_credit_type == 'instant') {
                                    $instant_selected = 'selected="selected"';
                                } else if ($referral_credit_type == 'on_first_ride') {
                                    $on_first_ride_selected = 'selected="selected"';
                                }
                            ?>
                            <li>
                                <div class="form_grid_12">
                                    <label class="field_title" for="referral_credit_type">REFERRAL CREDIT TYPE <span class="req">*</span></label>
                                    <div class="form_input">
                                        <select name="referral_credit_type" id="referral_credit_type" class="valid" required>
                                            <option></option>
                                            <option value="instant" <?php echo $instant_selected; ?>>Instant</option>
                                            <option value="on_first_ride" <?php echo $on_first_ride_selected; ?>>On First Ride</option>
                                        </select>
                                    </div>
                                </div>
                            </li>
