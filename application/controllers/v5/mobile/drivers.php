...
class Drivers extends MY_Controller {
        ...
        try {
            ...
            if ($email != '' && $password != '') {
                if (valid_email($email)) {
                    $checkDriver = $this->driver_model->get_selected_fields(DRIVERS, array('email' => strtolower($email), 'password' => md5($password)), array('email', 'user_name', 'mobile_number','push_notification','status', 'dail_code', 'outstanding_amount'));
                    if ($checkDriver->num_rows() == 1) {
                        if ($checkDriver->row()->status == 'Active') {
                            ...
                            if (!empty($push_data)) {
                                ...
                                $push_data['sso_token'] = $sso_token;
                                $this->driver_model->update_details(DRIVERS, $driverUpdateData, $push_data);
                                $this->driver_model->update_details(DRIVERS, $push_data, array('_id' => new \MongoId($checkDriver->row()->_id)));
                            } else {
                                $driverUpdateData = array(
                                    'sso_token' => $sso_token
                                );
                                $this->driver_model->update_details(DRIVERS, $driverUpdateData, array('_id' => new \MongoId($checkDriver->row()->_id)));
                            }
                            
                            $is_voda_customer = ($this->is_voda_customer($checkDriver->row()->dail_code, $checkDriver->row()->mobile_number) ? true : false);
                            $driverUpdateData = array(
                            ...
                            if (isset($checkDriver->row()->outstanding_amount)) {
                                $outstanding_amount = $checkDriver->row()->outstanding_amount;
                            } else {
                            ...
                            $returnArr['country_code'] = $checkDriver->row()->dail_code;
                            $returnArr['national_phone_number'] = $checkDriver->row()->mobile_number;
                            $returnArr['is_voda_customer'] = ($is_voda_customer ? 'true' : 'false');
        ...
        try {
            ...
            if ($chkValues >= 2) {
                ...
                if ($driver->num_rows() == 1) {
                    $ride = $this->driver_model->get_selected_fields(RIDES, array('ride_id' => $ride_id), array('ride_id', 'ride_status', 'booking_information', 'user.id', 'location.id', 'coupon_used', 'coupon', 'est_pickup_date', 'commission_percent','fare_breakup'));
                    $dataArr = array('last_accept_time' => new \MongoDate(time()));
                    $this->driver_model->update_details(DRIVERS, $dataArr, array('_id' => new \MongoId($driver_id)));
                    if ($ride->num_rows() == 1) {
                        if ($ride->row()->ride_status == 'Booked') {
                            $rider = $this->driver_model->get_selected_fields(USERS, array('_id' => new \MongoId($ride->row()->user['id'])), array('_id', 'push_type', 'push_notification_key', 'email', 'country_code', 'phone_number'));
                            if ($rider->num_rows() > 0) {
                                ...
                                if ($checkBooked->num_rows() > 0 && $availablity === true) {
                                    ...
                                    if (isset($rider->row()->push_type)) {
                                        if ($rider->row()->push_type != '') {
                                            ...
                                            if ($rider->row()->push_type == 'ANDROID') {
                                                if (isset($rider->row()->push_notification_key['gcm_id'])) {
                                                    if ($rider->row()->push_notification_key['gcm_id'] != '') {
                                                        $this->sendNewPushNotification($rider->row()->push_notification_key['gcm_id'], $message, 'reserved_ride_confirmed', 'ANDROID', $options, 'USER');
                                                    }
                                                }
                                            }
                                            if ($rider->row()->push_type == 'IOS') {
                                                if (isset($rider->row()->push_notification_key['ios_token'])) {
                                                    if ($rider->row()->push_notification_key['ios_token'] != '') {
                                                        $this->sendNewPushNotification($rider->row()->push_notification_key['ios_token'], $message, 'reserved_ride_confirmed', 'IOS', $options, 'USER');
                                                    }
                                                }
                                            }
                                        }
    ...
    public function start_upcoming_ride() 
    {
        ...
        try {
            ...
            if ($chkValues >= 3) {
                ...
                $checkRide = $this->driver_model->get_selected_fields(RIDES, array('ride_id' => $ride_id), array('ride_id', 'ride_status', 'booking_information', 'user.id', 'location.id', 'coupon_used', 'coupon', 'est_pickup_date', 'commission_percent','fare_breakup', 'driver.id'));
                ...
                if (!empty($routes))
                {
                    ...
                    if (isset($userVal->row()->push_type)) {
                        if ($userVal->row()->push_type != '') {
                            $message = $this->format_string('The driver is on its way', 'user_trip_started');
    ...
    public function location_arrived() {
        ...
        try {
            ...
            if ($chkValues >= 1) {
                $checkDriver = $this->app_model->get_selected_fields(DRIVERS, array('_id' => new \MongoId($driver_id)), array('email','loc', 'vehicle_model', 'driver_name', 'vehicle_number'));
                if ($checkDriver->num_rows() == 1) {
                    ...
                    if ($checkRide->num_rows() == 1) {
                        if ($checkRide->row()->ride_status == 'Confirmed') {
                            ...
                            $this->sms_model->driver_arrived($userVal->row(), $checkDriver->row());
    ...
    public function begin_ride() {
        ...
        try {
            ...
            if ($driver_id !='' && $ride_id !='' && $pickup_lat !='' && $pickup_lon !='' && $drop_lat !='' && $drop_lon !='') {
                $checkDriver = $this->app_model->get_selected_fields(DRIVERS, array('_id' => new \MongoId($driver_id)), array('email','last_accept_time'));
                if ($checkDriver->num_rows() == 1) {
                    $dataArr = array('last_begin_time' => new \MongoDate(time()));
                    $this->driver_model->update_details(DRIVERS, $dataArr, array('_id' => new \MongoId($driver_id)));
                    $this->mileage_model->update_mileage_system($driver_id,$checkDriver->row()->last_accept_time->sec,'customer-pickup',$distance,$this->data['d_distance_unit'],$ride_id);
    ...
    public function end_ride() {
        ...
        try {
           ...
           if ($interrupted == 'YES' || $interrupted != 'YES') {
            ...
            if ($chkValues >0) {
                $checkDriver = $this->app_model->get_selected_fields(DRIVERS, array('_id' => new \MongoId($driver_id)), array('email','last_begin_time'));
                if ($checkDriver->num_rows() == 1) {
                    ...
                    if ($checkRide->num_rows() == 1) {
                        ...
                        $dataArr = array('last_online_time' => new \MongoDate(time()));
                        $this->driver_model->update_details(DRIVERS, $dataArr, array('_id' => new \MongoId($driver_id)));
                        $this->mileage_model->update_mileage_system($driver_id,$checkDriver->row()->last_begin_time->sec,'customer-drop',$distance,$this->data['d_distance_unit'],$ride_id);
