...
class Mpesa_api extends MY_Controller {
    ...
    public function user_recharge_wallet() 
    {
        try {
            ...
            if (isset($user_id) && empty($user_id) && isset($amount) && empty($amount)) {
                ...
                if ($userResult->num_rows() == 1) {
                    ...
                    if ($loginResult['status'] == 1) {
                        ...
                        if ($mpesaTransaction['status'] === true) {
                            $walletUpdateResult = $this->wallet_recharge_model->update($user_id, $amount, $third_party_reference);
                            if ($walletUpdateResult['status'] === true) {
                                $add_tx = $this->mpesa_transaction_model->add($user_id, 'rider', $amount, $third_party_reference);
                                if ($add_tx['status']) {
                                    $returnArr['status'] = '1';
                                    $returnArr['insight_reference'] = $third_party_reference;
        ...
        try {
            ...
            if (isset($driver_id) && empty($driver_id)) {
                ...
                if ($driverResult->num_rows() == 1) {
                    ...
                    $request_data_items['amount'] = $driver->outstanding_amount;
                    ...
                    if ($loginResult['status'] == 1) {
                        ...
                        $mpesaTransaction = $this->_transaction($loginResult['SessionId'], $request_data_items, $third_party_reference);
                        if ($mpesaTransaction['status'] === true) {
                            $add_tx = $this->mpesa_transaction_model->add($driver_id, 'driver', $request_data_items['amount'], $third_party_reference);
                            if ($add_tx['status']) {
                                $returnArr['status'] = '1';
    ...
    public function get_transaction_status()
    {
        ...
        try {
            ...
            $third_party_reference = trim($this->input->post('insight_reference'));
            $user_id = trim($this->input->post('user_id'));
            $driver_id = trim($this->input->post('driver_id'));
            ...
            if (!empty($login_token) && !empty($third_party_reference) && (!empty($user_id) || !empty($driver_id))) {
                ...
                $transactionResult = $this->mpesa->transaction_status($login_token, $third_party_reference, $phone_number);
                if ($transactionResult['message'] === 'Success') {
                    if (isset($user_id) && !empty($user_id)) {
