...
class Billing extends MY_Controller {
	...
	public function generate_billing(){
		...
		*/  
		...
		if($hasBIlling=== TRUE){		
			...
			$selectFields= array('email','image','driver_name','vehicle_number','vehicle_model','no_of_rides','cancelled_rides','mobile_number','dail_code','currency','currency_symbol');
			...
			if($driverDetails->num_rows()>0){
				...
				foreach ($driverDetails->result() as $driver){
					...
					if(isset($driver->currency_symbol)){
						$currencysymbol=$driver->currency_symbol;
					}
	...
	public function update_driver_outstanding_amount()
	{	
		$driverDetails = $this->driver_model->get_selected_fields(DRIVERS, array(), array('_id'));
		if ($driverDetails->num_rows() > 0) {
			foreach ($driverDetails->result() as $driver) {
				$driver_id = (string) $driver->_id;
				$rideDetails = $this->revenue_model->get_ride_details($driver_id);
				$in_site = 0;
				$site_earnings = 0;				
				if (count($rideDetails['result']) === 1) {
					$site_earnings = $rideDetails['result'][0]['site_earnings'];
					if (isset($rideDetails['result'][0]['amount_in_site'])) {
						$in_site = $rideDetails['result'][0]['amount_in_site'];
					}
				}
				$outstanding_amount = $site_earnings - $in_site;
				$updateDriver = array('outstanding_amount' => 
					array(
						'amount' => $outstanding_amount,
						'updated_on' => new MongoDate(time())
					)
				);
				$this->app_model->update_details(DRIVERS, $updateDriver, array('_id' => new \MongoId($driver_id)));
			}
		} else {
